...
/// Per-column advice values passed to [`gen_snark_from_base`].
pub type AdviceColumns = halo2_base::halo2_proofs::plonk::AdviceColumns<Fr>;
...
pub fn synthesize_witness_shplonk<ConcreteCircuit: Circuit<Fr>>(
    ...
    Vec<halo2_base::halo2_proofs::plonk::InstanceSingle<G1Affine>>,
    Vec<halo2_base::halo2_proofs::plonk::AdviceSingle<G1Affine>>,
    Vec<Fr>,
) {
    let instances_ref: Vec<&[Fr]> = instances.iter().map(Vec::as_slice).collect();
    let mut transcript =
        PoseidonTranscript::<NativeLoader, Vec<u8>>::from_spec(vec![], POSEIDON_SPEC.clone());
    let rng = StdRng::seed_from_u64(0);
    halo2_base::halo2_proofs::plonk::synthesize_witness::<_, ProverSHPLONK<_>, _, _, _, _>(
        params,
        pk,
        &[circuit],
...
/// Generates a SHPLONK snark from already synthesized advice columns as generated by halo2-base edsl
///
/// The caller must supply one entry per advice column of `pk`'s constraint system,
/// each of size `params.n()`, laid out exactly as `BaseCircuitBuilder` synthesis
/// would produce.
pub fn gen_snark_from_base(
    params: &ParamsKZG<Bn256>,
    pk: &ProvingKey<G1Affine>,
    advice: AdviceColumns,
    instances_owned: Vec<Vec<Fr>>,
) -> Snark {
    let protocol = compile(
        params,
        pk.get_vk(),
